...
function itemPrice(n_price, s_price) {
  return `<dd class="item-price">${salePrice(
    s_price.slice(0, -1)
  )}<p class='unit'>원</p>${originalPrice(n_price)}</dd>`;
}
...
function badgeTpl(badge) {
  return badge.reduce(
    (prev, curr) => prev + `<div class="event"><span>${curr}</span></div>`,
    ""
  );
}
...
export function itemListTpl(itemsData) {
  itemsData =
    Object.prototype.toString.call(itemsData) === "[object Object]"
      ? itemsData.items
      : itemsData;
  return itemsData.reduce(
    (prev, curr) =>
      prev + `<li class="items"><a href="#">${itemsTpl(curr)}</a></li>`,
    ""
  );
}
